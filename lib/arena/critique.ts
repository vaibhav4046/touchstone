...
async function fromModel(material: CritiqueMaterial): Promise<readonly Disagreement[]> {
  ...
  const outcome = await complete({
    ...
    user: [
      ...
      "-----",
      "",
      'Return [{"code":"SHORT_UPPER_SNAKE_ID","statement":"the specific disagreement, at least 30 characters","quote":"verbatim span copied from the listing or the trial record","falsifiedBy":"what the seller could show to defeat this point"}]',
      "At most three entries. Copy quotes character for character or they will be discarded.",
    ].join("\n"),
    maxTokens: 900,
